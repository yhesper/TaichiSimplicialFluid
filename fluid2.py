...
meta = Patcher.mesh2meta("meshes/sphere_large.obj", relations=["ev", "vv", "vf", "fv", "ef", "fe", "ve", "ff"])
...
window = ti.ui.Window("simp fluid", (win_x, win_y), vsync=True)
...
while True:
    ...
    #     mycam.update_by_mouse()
    #     cp = mycam._camera_pos
    #     la = mycam._lookat_pos
    #     camera.position(cp[0], cp[1], cp[2])
    ...
    canvas.scene(scene)
    if (write_image):
        window.write_image("screenshots/cow.png")
        write_image = False
